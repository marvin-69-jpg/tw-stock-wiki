...
>
> **2026-04-18 Round 43（ripple 擴散機制頁）**：`wiki/mechanisms/active-bond-etf-d-suffix.md` 5 檔 → **7 檔**（加 00981D 三段保管費斷點分歧 + 00985D 外資投等費率趨同 + 外資首檔 = D 字尾投等模式、Round 42 外資 flat signature 最終推翻）；`wiki/mechanisms/issuer-divergence-logic.md` 10 家 7 維 → **11 家 8 維**（**新 Type 8 雙維分歧 = 中信** 00983A/00995A/00981D、Type 3 地域移除中信、貝萊德第 15 家但 1 檔暫不計入母體）；`wiki/mechanisms/active-etf-fee-disclosure.md` 新增 Round 42-43 最終推翻段（外資投等 D 字尾 100% 階梯、外資股票/非投等才 flat、**flat vs 階梯 ≈ 策略類型偏好非國籍決定**）。本輪純機制頁 ripple、無新 ETF / issuer primary。
>
...
| [[wiki/mechanisms/issuer-divergence-logic\|同發行商內分歧邏輯]] | **Round 43 擴充為 11 家 8 維**（Round 41 中信升維 Type 8 雙維 + Round 42 貝萊德第 15 家但 1 檔暫不列）：主題深度（群益）/ 主動裁量權（野村）/ 地域純地域（統一台新復華摩根）/ 風險等級傘型（富邦）/ 資產類型傘型（復華）/ 費率無分歧（安聯）/ 信用等級非傘型（聯博）/ **雙維分歧地域+資產類型（中信、00983A/00995A/00981D、本研究首個雙維）**、**外資首檔 ETF = D 字尾投等模式**（聯博 + 貝萊德雙例）、外資分歧維度最多元（4/11） | divergence-logic, fee-structure, product-line-strategy, cross-issuer-comparison, credit-quality-divergence, dual-axis-divergence | 2026-04-18 |
| [[wiki/mechanisms/active-bond-etf-d-suffix\|主動債券 ETF（D 字尾）機制]] | **Round 40-42 修正為 7 檔**（加入 00981D 中信 + 00985D 貝萊德）：信用品質單調費率光譜（0.45→0.80%）、**月配 7/7 通例 vs A 字尾月配 0**、三段階梯保管費 2 例（富邦 100/300 + 中信 100/200 斷點分歧）、傘型子基金保管銀行獨立（復華）、非投等 10.07% 年化殖利率研究最高、**Round 42：外資投等 D 字尾費率趨同**（聯博 00980D + 貝萊德 00985D 保管費 100% 相同 0.12/0.08% 斷點 300 億）、**外資首檔 = D 字尾投等模式**（聯博 + 貝萊德雙例）、**TPEx 佔 4/7（57%）**、Round 40 TPEx audit 盲區閉合 | active-bond-etf, d-suffix, fixed-income, monthly-dividend, credit-risk, tiered-custody, nav-composition-daily, tpex-blindspot, foreign-ig-convergence | 2026-04-18 |
