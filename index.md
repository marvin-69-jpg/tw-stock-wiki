...
| [[wiki/mechanisms/issuer-divergence-logic\|同發行商內分歧邏輯]] | 9 家 ≥2 檔主動 ETF 投信共 6 種分歧維度：主題深度（群益）/ 主動裁量權（野村）/ 地域（統一台新中信復華摩根）/ 風險等級傘型（富邦）/ 資產類型傘型（復華）/ 無分歧（安聯）、跨 Round 11-31 累積歸納 | divergence-logic, fee-structure, product-line-strategy, cross-issuer-comparison | 2026-04-18 |
| [[wiki/mechanisms/active-bond-etf-d-suffix\|主動債券 ETF（D 字尾）機制]] | 4 檔 D 字尾完整歸納：信用品質單調費率光譜（0.55→0.80%）、本土階梯 / 外資 flat signature 延續、**月配 4/4 通例 vs A 字尾月配 0**、三段階梯保管費首見（富邦）、傘型子基金保管銀行獨立（復華）、非投等 10.07% 年化殖利率研究最高 | active-bond-etf, d-suffix, fixed-income, monthly-dividend, credit-risk, tiered-custody | 2026-04-18 |
